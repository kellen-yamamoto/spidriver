...
/*---------- SYSFS Interface ------------*/
static ssize_t spidev_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "TESTING\n");
}

static DEVICE_ATTR(spidev, S_IRUGO, spidev_show, NULL);
...
static int __init add_avraudio_device_to_bus(void)
{
	...
	if (pdev) {
		...
		if (status < 0) { 
			...
			device_create_file(&spi_device->dev, &dev_attr_spidev);
		}
	}
